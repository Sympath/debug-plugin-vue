...
| 配置项         | 类型                | 作用                                                         | 是否必填 |
| -------------- | ------------------- | ------------------------------------------------------------ | -------- |
| getMappWinodow | Function            | 获取子应用的全局变量 （如果不需要承接微应用可以不传递）；默认参数会传递一个对象，为所有搜集到的vue组件实例，key：自定义的key；val：对应组件实例； | 否       |
| isDev          | Function \| Boolean | 判断是否是开发环境 如果函数返回为true才接入插件（传入true也行） | 是       |
...
##### 使用效果展示
![1](https://tva1.sinaimg.cn/large/008i3skNly1gq1n49nffgg30b404bqv5.gif)
![2](https://tva1.sinaimg.cn/large/008i3skNly1gq1n47yd21g309103ix3p.gif)
![3](https://tva1.sinaimg.cn/large/008i3skNly1gq1n4c38yqg309103ix4n.gif)
![4](https://tva1.sinaimg.cn/large/008i3skNly1gq1n4wcorag309103ie5a.gif)
...
let vueDebugPlugin = vueDebugPluginFn({
    ...
    // isMapp: true 如果是子应用 需要设置为true  无微应用可以不传
})
